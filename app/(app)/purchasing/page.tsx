  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-slate-500">Suppliers and purchase orders for inventory restocking.</p>
      </div>
      <section className="rounded-lg border border-slate-200 bg-white p-5">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-medium text-slate-900">Suppliers</h2>
          <details className="relative">
            ...
            >
              ...
              <div>
                ...
                <input name="contact_phone" className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-600">Contact Email</label>
                <input type="email" name="contact_email" className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" />
              </div>
          ...
          <details className="relative">
            ...
            >
              <div>
                ...
                <select name="supplier_id" required className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm">
              ...
              <tr key={po.id} className="border-b border-slate-100">
                ...
                <td className="py-2 text-right">
                  <form action={updatePurchaseOrderStatus} className="inline-flex items-center gap-2">
                    ...
                    <select name="status" defaultValue={po.status} className="rounded-md border border-slate-300 px-1.5 py-1 text-xs">
                      {poStatuses.map((s) => (
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-medium text-slate-900">Purchase Order Items</h2>
          <details className="relative">
            <summary className="cursor-pointer rounded-md bg-slate-900 px-3 py-1.5 text-sm font-medium text-white">
              Add Item
            </summary>
            ...
            >
              <div>
                ...
                <select name="purchase_order_id" required className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm">
              ...
              <div>
                ...
                <select name="inventory_item_id" required className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm">
              ...
              <div>
                ...
                <input type="number" step="0.01" name="quantity" required className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-600">Unit Price (₹)</label>
                <input type="number" step="0.01" name="unit_price" required className="mt-1 w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm" />
              </div>
              <button type="submit" className="w-full rounded-md bg-slate-900 px-3 py-1.5 text-sm font-medium text-white">
                Add Item
              </button>
